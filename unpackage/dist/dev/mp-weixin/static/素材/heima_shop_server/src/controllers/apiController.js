  ...
  req.db.driver.execQuery(sql, (err, data) => {
    ...
    if (err) {
      ...
      return
		...
		// http://vue.studyit.io
	  data.forEach(item => item.thumb_path = 'http://localhost:8082' + item.thumb_path)
  ...
  req.db.driver.execQuery(sql, (err, datas) => {
    ...
    if (err) {
	  ...
	  datas.forEach(item => {
		// http://ofv795nmp.bkt.clouddn.com/
		// http://vue.studyit.io
		item.img_url = 'http://localhost:8082' + item.img_url
	  })
...
exports.getimage = (req, res) => {
  ...
  req.db.driver.execQuery(sql, (err, data) => {
	...
	data.forEach(item => {
		// http://ofv795nmp.bkt.clouddn.com/
		// http://vue.studyit.io
		item.src = 'http://localhost:8082' + item.src
	})
